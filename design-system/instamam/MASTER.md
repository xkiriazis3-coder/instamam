...
| Relying on a `<dialog>`'s native Escape when it was opened inside `startViewTransition()` | Verified with a real key press: the Escape keydown reached the page with `defaultPrevented: false`, **no `cancel` event fired**, and the dialog stayed open. A keyboard user could enter the photo viewer with no key to leave it. Escape is now handled explicitly on the dialog. Native close-watcher behaviour is not guaranteed once the open happens inside a view transition |
| Responsive overrides placed before the components they override | A media query adds **no specificity**. A `@media (max-width: 380px)` block sitting near the top of the stylesheet lost to the component rules defined further down — only the custom property took effect, so the "fix" silently did nothing and the header stayed clipped. Small-screen overrides live at the END of `style.css` |
| Untested 320px viewport | The header row measured 363px against a 320px viewport. `body` has `overflow-x: hidden`, so it clipped rather than scrolled and the menu button lost **23 of its 44px** off the right edge — half the navigation control unreachable on an iPhone SE. Test 320, not just 375 |
